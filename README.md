...
- **选手池** (153 人): 24 支知名战队现役选手 + CS2 Major 冠军 + 2018 以来 CSGO Major 冠军 + 知名教练 (zonic/B1ad3 级别) + 4 名解说员
- **每名选手 6 项属性**: 国家、战队、年龄、Major 冠军数、位置（步枪手/狙击手/教练/解说）、最高 HLTV Top20 排名
- **智能猜测栏**: 输入自动补全匹配，回车选择第一个匹配，上下方向键在候选间导航，提交后自动清空
- **双通道通信**: 优先 WebSocket 实时推送；若 WS 不可用（如 cloudflared 不转发 Upgrade 请求）自动降级为 HTTP 轮询，保证任何网络环境可玩
- **抢 N 赛制**: 可选抢 1/2/3/4
- **每小局**: 2 分钟 / 8 次猜测
